...
class DataCache:
    ...
    async def load_leagues(self) -> None:
        ...
        async with self._leagues_lock:
            now = datetime.now()
            if (
                self._leagues_loaded
                and self._leagues_loaded_at
    ...
    async def load_popular_teams(self) -> None:
        ...
        async with self._teams_lock:
            now = datetime.now()
            if (
                self._teams_popular_loaded
                and self._teams_popular_loaded_at
    ...
    async def load_all_data(self) -> None:
        ...
        try:
            ...
            await asyncio.gather(self.load_leagues(), self.load_teams(), return_exceptions=True)
    ...
    async def get_teams(self) -> List[Dict[str, Any]]:
        ...
        now = datetime.now()
        if (
            not self._teams_popular_loaded
        ...
        for league_info in major_leagues:
            ...
            try:
                ...
                teams_data = client.get_rankings(
                    league=league_id, game_class=game_class, season=season
                )
                ...
                for team in teams[:15]:  # Try up to 15 teams per league to find ones with rosters
                    ...
                    try:
                        ...
                        logger.info(
                            f"  → Found {len(players)} players in team {team_id} ({team_name})"
                        )
                        ...
                        for player in players:
                            ...
                            if player_id not in self._players:
                                ...
                                self._players[player_id] = {
                                    ...
                                    "text": player_name,
                                    "teams": [
                                        {"id": team_id, "name": team_name, "league": league_name}
                                    ],
            ...
            for game_class_info in game_classes[:1]:
                ...
                try:
                    ...
                    for season_attempt in [current_season, current_season - 1]:
                        try:
                            logger.info(
                                f"Fetching games for {league_name} (league={league_id}, game_class={game_class}, season={season_attempt})..."
                            )
                            ...
                            if season_attempt == current_season:
                                logger.debug(
                                    f"  Season {season_attempt} failed: {e}, trying previous season..."
                                )
                            else:
                    ...
                    for game in games[:5]:  # Process 5 games per league
                        ...
                        try:
                            ...
                            for is_home in [1, 0]:  # 1=home, 0=away
                                ...
                                logger.info(
                                    f"    → Found {len(players)} players in {team_type} lineup"
                                )
                                ...
                                for player in players:
                                    ...
                                    if player_id not in self._players:
                                        ...
                                        if game_id not in self._players[player_id].get("games", []):
                                            self._players[player_id].setdefault("games", []).append(
                                                game_id
                                            )
                    ...
                    logger.debug(
                        f"Could not process games for league {league_id}/{game_class}: {e}"
                    )
                    continue
    ...
    async def build_comprehensive_player_index(self) -> None:
        ...
        async with self._players_lock:
            ...
            try:
                ...
                logger.info(
                    f"✓ Indexed {len(self._players)} total players "
